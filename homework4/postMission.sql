...
/-- 아무 값이나 INSERT 해보기 --/
INSERT INTO post  VALUES (21,'juno','newTitle','newContent','2020-05');
/-- postIdx가 3인 데이터 조회하기 --/
SELECT * FROM post WHERE postIdx=3;
/-- postIdx가 2인 post 개체들을 모두 출력하기 --/
SELECT * FROM post WHERE posdtIdx=2;
/-- 선택 ) post.sql 17 ~ 26을 실행시켰다면 userIdx가 4인 post+user 개체를 JOIN으로 출력해보기 --/
SELECT * FROM post JOIN user ON post.postIdx = user.userIdx;

/-- postIdx가 2인 데이터 날짜 현재로 수정하기 --/
UPDATE post SET createdAt = '20/05/2020' WHERE posdtIdx=2;
/-- postIdx가 4인 데이터 지우기 --/
